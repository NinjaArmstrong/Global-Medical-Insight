...
export const articles: Article[] = [
  ...
  {
    id: "1101",
    title: { ja: "【米国】FDA QMSR：2月2日施行・クラスIII機器の出荷停止リスク", en: "[US] FDA QMSR: Shipment Stop Risk from Feb 2 Strict Enforcement" },
    region: "North America",
    date: "2026-01-07",
    riskLevel: "High",
    scenario: { ja: "2026年2月2日のQMSR施行において、FDAは移行期間を設けず即時適用する方針である。ISO 13485未対応のDHF（設計開発ファイル）は、ワーニングレターの直接原因となる。", en: "FDA declares 'No Transition Period' for Feb 2 QMSR enforcement. ISO 13485 non-compliant Design Controls (DHF) trigger immediate Warning Letters." },
    impact: { ja: "特に循環器・整形外科領域の**クラスIIIインプラント**において、旧QSRに基づくリスク管理文書が残存している場合、是正完了まで**無期限の出荷停止（Shipment Stop）**措置を受けることは確実である。", en: "Class III implants with old QSR Risk Mgmt files face mandatory Recall and indefinite shipment stops until remediation." },
    action: { ja: "品質保証担当役員（CQAO）は、QSIT監査を直ちに中止し、新基準での模擬査察へ切り替える指示を出すとともに、不適合是正のための**緊急予算枠（予備費）の決裁**を行うことが推奨されます。", en: "CQAO should halt QSIT audits, order switch to new mock inspections, and urgently approve remediation budgets for non-conformities." },
  ...
  {
    id: "1102",
    title: { ja: "【欧州】AI法：適合性証明不備によるMDR認証失効", en: "[Europe] AI Act: MDR Certificate Revocation Risk" },
    region: "Europe",
    date: "2026-01-07",
    riskLevel: "High",
    scenario: { ja: "2026年8月のAI法適用に向け、Notified BodyはMDR技術文書の再審査を開始した。AIの透明性が証明できない場合、既存のCEマーキングは無効化される運用である。", en: "Notified Bodies begin MDR Tech File re-reviews for Aug 2026 AI Act. Existing CE Marks voided if AI 'Explainability' proof is insufficient." },
    impact: { ja: "**画像診断支援（CAD）やトリアージAI**において、「ブラックボックス性」が解消されない限り、市場からの**強制回収（Recall）**および欧州売上の喪失が不可避となる。", en: "'Black Box' CAD systems face forced market withdrawal, causing instant revenue loss across Europe." },
    action: { ja: "開発本部長は、AIモデルの「解釈可能性レポート」作成を最優先事項とし、8月までに第三者検証機関の**適合証明書を取得するプロジェクト**を直轄することが賢明です。", en: "Head of R&D must directly oversee 'Transparency Report' creation and secure third-party validation by August." },
    url: "https://digital-strategy.ec.europa.eu/en/policies/regulatory-framework-ai"
  },
  {
    id: "1103",
    title: { ja: "【日本】医療DX予算：サイバー安保要件による商流喪失", en: "[Japan] Medical DX: Vendor Shakeout via Cybersecurity Reqs" },
    region: "East Asia",
    date: "2026-01-07",
    riskLevel: "Medium",
    scenario: { ja: "2026年度診療報酬改定の「医療DX推進体制整備加算」要件として、SBOM（ソフトウェア部品表）の提供が義務化された。未対応システムは加算対象外となる。", en: "Facilities using systems without SBOMs are disqualified from 'Medical DX Premiums' in 2026 Fee Revision." },
    impact: { ja: "**中小規模の電子カルテや生体モニタ**ベンダーは、SBOM未提供が顧客病院の減収（加算逸失）直結するため、**契約解除（リプレース）**の対象となり、市場から淘汰される。", en: "SME EHR/System vendors lacking vulnerability processes hinder hospital revenue, facing contract termination and market exit." },
    action: { ja: "事業責任者は、自社製品の**「SBOM対応証明書」**を営業ツールとして全拠点に配布し、競合他社の不備を指摘する形で**リプレース提案を加速**させることが得策です。", en: "Business Unit Head should distribute 'SBOM Compilance Certs' for aggressive replacement sales targeting non-compliant competitors." },
    url: "https://www.mhlw.go.jp/stf/2026budget_dx.html"
  },
  {
    id: "1104",
    title: { ja: "【インドネシア】ハラール強制：未承認品のカテーテル市場喪失", en: "[Indonesia] Halal: Stock Disposal and Channel Loss" },
    region: "Southeast Asia",
    date: "2026-01-07",
    riskLevel: "High",
    scenario: { ja: "2026年10月のハラール義務化により、BPJPHの審査渋滞（ボトルネック）が確実視される。未認証品は事実上の禁輸措置（通関不可）となる。", en: "BPJPH overwhelmed ahead of Oct 2026 mandate. 'Avg 14-month' delay from application to certification." },
    impact: { ja: "特に**動物由来原料を使用しないカテーテルや輸液セット等の消耗品メーカー**において、「対象外である」という誤認が致命傷となり、現地シェアを失うリスクがある。", en: "Animal-derived catheters/sutures without certs by Oct face customs seizure and total disposal (multi-million dollar loss)." },
    action: { ja: "薬事部門および現地法人は、直ちに製品成分表（BOM）のスクリーニングを行い、相互認証機関を通じた**申請枠の仮押さえ**を行うことがリスク回避の最善策となります。", en: "Production must switch to JAKIM apps immediately and execute 'Forward Stocking' strategy to ship post-Oct demand by Sept." },
    url: "https://bpjph.halal.go.id/"
  },
  {
    id: "1105",
    title: { ja: "【インド】CDSCO：港湾デムラージュによるIVD試薬廃棄損", en: "[India] CDSCO: Port Demurrage and Agent Termination" },
    region: "South Asia",
    date: "2026-01-07",
    riskLevel: "High",
    scenario: { ja: "輸入許可証（Form MD-15）の原本照合厳格化により、港湾部での貨物滞留とそれに伴う**高額なデムラージュ（超過保管料）**が発生している。", en: "Customs strictly verifies MD-15 originals. Non-compliant cargo detained indefinitely in bonded areas." },
    impact: { ja: "**体外診断用医薬品（IVD）や試薬**など、温度管理必須の製品において、通関遅延はそのまま**製品廃棄（廃棄損）**に直結し、利益率を大きく圧迫する要因となる。", en: "Weeks of detention for high-turnover implants cause massive demurrage fees, pushing margins into the red." },
    action: { ja: "SCM部門は、出荷前に現地代理人とインボイス記載内容の完全一致確認を徹底するフローを導入し、**無用なトラブルを未然に防ぐ体制構築**が急務です。", en: "SCM must double-check shipping docs; Local entity should station staff at ports to present Reg proofs immediately." },
    url: "https://cdsco.gov.in/"
  },
  {
    id: "1106",
    title: { ja: "【サウジ】NUPCO入札：現地化要件による汎用品の市場退場", en: "[Saudi] NUPCO: Tender Disqualification via Localization" },
    region: "Middle East",
    date: "2026-01-07",
    riskLevel: "Medium",
    scenario: { ja: "NUPCO入札における技術評価で、現地化比率（Local Content）の加点が10%から25%へ大幅に引き上げられた。", en: "NUPCO introduces criteria effectively delisting vendors with '<30% Local Content' from 2026 tenders." },
    impact: { ja: "価格競争が激しい**シリンジやガーゼ等の汎用品**において、現地工場を持たないメーカーは入札要件を満たせず、サウジ市場からの**退場（撤退）**を余儀なくされる可能性が高い。", en: "Consumables (IV sets) makers without local assembly face total exit from Saudi public market (70% of share)." },
    action: { ja: "海外事業本部は、現地代理店の工場を活用した**委託組立（Knock-down生産）への切り替え**について、採算性シミュレーションを早急に開始することが推奨されます。", en: "Global Exec must decide on KD plant JV with agents this month to qualify for next tender PQ." },
    url: "https://www.nupco.com/"
  },
  {
    id: "1107",
    title: { ja: "【ケニア】デジタルヘルス：非互換機器の入札参加資格喪失", en: "[Kenya] Digital Health: Purge of Non-Compatible Devices" },
    region: "Africa",
    date: "2026-01-07",
    riskLevel: "Low",
    scenario: { ja: "ケニア保健省は、国家EHRへのデータ送信機能を持たない検査機器について、公立病院での新規購入を禁止する措置を開始した。", en: "MoH bans purchase/renewal of diagnostic devices lacking data transmission to National EHR in public hospitals." },
    impact: { ja: "**スタンドアローン型のPOCT機器（血糖値測定器など）**は、技術仕様書（Technical Spec）を満たせないため、**公的入札からの排除**対象となる。", en: "Standalone POCT/Lab devices fail Tech Specs and are excluded from all Kenyan public tenders." },
    action: { ja: "技術本部は、現地EHRベンダーとのAPI連携仕様を入手し、**HL7 FHIR出力対応の外付けゲートウェイ**開発に着手することが、市場残留のための必須条件です。", en: "Tech Div must demand API specs from local vendors and develop 'External Gateways' for HL7 FHIR output immediately." },
    url: "https://health.go.ke/"
  },
  {
    id: "1108",
    title: { ja: "【メキシコ】国産化優遇：価格競争力喪失とシェア急落", en: "[Mexico] Domestic Preference: Share Collapse from Price Gap" },
    region: "Latin America",
    date: "2026-01-07",
    riskLevel: "Medium",
    scenario: { ja: "メキシコ政府は国内製造品に対し、入札価格で15%の優遇（競合より高くても落札可）を与える大統領令を完全施行した。", en: "Decree grants 15% price preference (win even if higher price) to domestic goods in tenders." },
    impact: { ja: "**日本からの完成品輸出に依存するジェネリック医薬品**は、価格競争力を完全に喪失するため、IMSS（社会保険庁）入札での**落札率がゼロ**になるリスクがある。", en: "finished goods exports (Oncology/Antibiotics) lose all price competitiveness against local generics, hitting 0% win rate in IMSS tenders." },
    action: { ja: "中南米地域統括は、現地CMOへのバルク供給契約を締結し、最終包装工程を現地化することで**「国産品ステータス」を獲得**する戦略への転換が推奨されます。", en: "LatAm Head must sign bulk supply deals with local CMOs to shift final pkg and gain 'Domestic Status' as the only survival strategy." },
    url: "https://www.gob.mx/cofepris"
  },
  {
    id: "1109",
    title: { ja: "【米国】IRA薬価交渉：フォーミュラリー除外による売上蒸発", en: "[US] IRA Negotiation: Sales Vanish via Formulary Exclusion" },
    region: "Global",
    date: "2026-01-07",
    riskLevel: "High",
    scenario: { ja: "IRA交渉価格の適用開始に伴い、大手PBMは高薬価の競合品をフォーミュラリー（推奨薬リスト）から完全に除外する動きを加速させている。", en: "With IRA prices effective, PBMs accelerate total exclusion of high-price competitors from formularies." },
    impact: { ja: "**糖尿病・抗凝固薬**領域の競合製品を持つメーカーは、民間保険市場へのアクセスを遮断され、米国売上の数割が**瞬時に消失（蒸発）**する。", en: "Firms with competing Diabetes/CV drugs face blocked access to US private market, risking instant vaporization of 30-40% of US sales." },
    action: { ja: "マーケットアクセス本部は、リベート競争からの脱却を掲げ、アウトカムに基づく**リスクシェアリング契約**をPBMに提案し、フォーミュラリー残留を勝ち取ることが賢明です。", en: "Market Access must pivot from rebate hikes to outcome-based risk-sharing deals to secure formulary retention." },
